...
void add_node(struct Graph *graph, uint64_t data) {
	graph->nodes = (struct Node **)realloc(
		graph->nodes, (graph->num_nodes + 1) * sizeof(struct Node *));
...
void add_edge(struct Graph *graph, struct Node *src, struct Node *dest,
			  int weight) {
	graph->edges = (struct Edge **)realloc(
		graph->edges, (graph->num_edges + 1) * sizeof(struct Edge *));
